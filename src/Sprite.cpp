...
auto Sprite::addFrame(Frame frame) -> int {
  if (this->frames.size() >= this->frames.capacity()) {
    Log.error("Sprite::addFrame(): Cannot add frame. Maximum",
              this->frames.capacity(), "reached");
    return -1;
  }

  if (frame.getWidth() != this->width || frame.getHeight() != this->height) {
    Log.error("Sprite::addFrame(): Frame dimensions", frame.getWidth(),
              frame.getHeight(), "do not match sprite dimensions", this->width,
              this->height);
    return -1;
  }
...
auto Sprite::getFrame(int frameNumber) const -> Frame {
  if (frameNumber < 0 || frameNumber >= this->frames.size()) {
    Log.warning("Sprite::getFrame(): Invalid frame number", frameNumber, "with",
                this->frames.size(), "frames");
    return {};
  }
...
auto Sprite::draw(int frameNumber, Vector position) const -> int {
  if (frameNumber < 0 || frameNumber >= this->frames.size()) {
    Log.warning("Sprite::draw(): Invalid frame number", frameNumber, "with",
                this->frames.size(), "frames");
    return -1;
  }
