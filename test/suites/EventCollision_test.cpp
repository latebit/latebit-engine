#include "EventCollision.h"

#include "../lib/test.h"

using namespace df;

void EventCollision_test() {
  EventCollision subject;
  assert("sets default object1", subject.getObject1() == nullptr);
  assert("sets default object2", subject.getObject2() == nullptr);
  assert_vector("sets default position", subject.getPos(), Vector());

  Object obj1, obj2;
  Vector pos(1, 1);
  subject = EventCollision(&obj1, &obj2, pos);
  assert("sets correct object1", subject.getObject1() == &obj1);
  assert("sets correct object2", subject.getObject2() == &obj2);
  assert_vector("sets correct position", subject.getPos(), pos);

  Object obj3;
  subject.setObject1(&obj3);
  assert("sets correct object1", subject.getObject1() == &obj3);

  Object obj4;
  subject.setObject2(&obj4);
  assert("sets correct object2", subject.getObject2() == &obj4);

  Vector pos2(2, 2);
  subject.setPos(pos2);
  assert_vector("sets correct position", subject.getPos(), pos2);
}